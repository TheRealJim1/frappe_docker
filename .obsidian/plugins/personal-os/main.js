...
// src/Component/Settings/SettingsTab.ts
var PersonalOSSettings = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian4.Setting(containerEl).setName("Config folder").setHeading();
    new StringSettings(
      containerEl,
      this.saveSettings,
    ...
    new ListSetting(
      ...
      containerEl.createEl("div"),
      this.randomEvents,
      "Start work random events",
      "Add a new tag for random event"
    );
    new import_obsidian4.Setting(containerEl).setName("Probability of random events").addText((text) => text.setValue(this.probabilityRandomEvents.toString()).setPlaceholder("10").onChange((value) => {
...
var setPriority = (p, t) => {
  ...
  if (text.includes("\u{1F53A}") && priority < 4)
    p.priority = 4;
  else if (text.includes("\u23EB") && priority < 3)
    p.priority = 3;
  else if (text.includes("\u{1F53C}") && priority < 2)
    p.priority = 2;
  else if (text.includes("\u{1F53D}") && priority < 1)
    p.priority = 1;
  else
    p.priority = 0;
  return true;
...
var isTimeBeforeCurrentTime = (t) => {
  ...
  if (timeSubstring) {
    return (0, import_obsidian5.moment)(timeSubstring, "HH:mm").isBefore((0, import_obsidian5.moment)());
  }
  return false;
};
...
var IsTodayTask = (t) => t.due && (0, import_obsidian5.moment)(t.due.ts).isSame((0, import_obsidian5.moment)(), "day") || t.scheduled && (0, import_obsidian5.moment)(t.scheduled.ts).isSame((0, import_obsidian5.moment)(), "day");
...
var IsNextPage = (p) => p.file.tasks.where(isNotCompletedOrCancelled).where((t) => t.start && (0, import_obsidian5.moment)(t.start.ts).isSameOrBefore((0, import_obsidian5.moment)(), "day") && !t.scheduled).where((t) => setPriority(p, t)).length > 0 || p.file.tasks.where((t) => t.start || t.due || t.scheduled || t.text.contains("[[Someday]]") || t.text.contains("#Someday")).where(isNotCompletedOrCancelled).length == 0 && p.file.tasks.where(isNotCompletedOrCancelled).where((t) => setPriority(p, t)).length > 0;
var isLate = (p) => p.file.tasks.where(isNotCompletedOrCancelled).where(
  (t) => t.due && (0, import_obsidian5.moment)(t.due.ts).isBefore((0, import_obsidian5.moment)(), "day") || t.scheduled && (0, import_obsidian5.moment)(t.scheduled.ts).isBefore((0, import_obsidian5.moment)(), "day")
).length > 0;
var isOpenNoTask = (p) => p.file.tasks.where(isNotCompletedOrCancelled).length == 0;
var isTaskFailed = (t) => isNotCompletedOrCancelled(t) && t.due && (0, import_obsidian5.moment)(t.due.ts).isBefore((0, import_obsidian5.moment)(), "day");
...
` + content);
    ...
    this.setFocus = (listOfFiles) => {
      ...
      const maxPriority = listOfFiles.values.reduce((max, item) => Math.max(max, item.priority), 0);
      const listOfPriorityFiles = listOfFiles.values.filter((file) => file.priority == maxPriority);
      const randomFile = this.focus ? this.focus : listOfFiles[Math.floor(Math.random() * listOfFiles.length)];
      this.focus = null;
...
var StartProcess = class {
  constructor(app, graph, inboxPages, configFolder) {
    this.setInboxPages = (inboxPages) => {
      const filteredPages = inboxPages.filter((p) => this.app.vault.getAbstractFileByPath(p) instanceof import_obsidian7.TFolder);
      this.inboxPages = filteredPages.length > 0 ? filteredPages : void 0;
    };
    this.setConfigFolder = (configFolder) => {
      this.configFolder = this.app.vault.getAbstractFileByPath(configFolder || "/") instanceof import_obsidian7.TFolder ? configFolder : "";
      if (this.configFolder === "/" || this.configFolder === "")
        this.configFolder = "";
      else if (!this.configFolder.endsWith("/"))
        this.configFolder += "/";
    ...
    this.findNextFile = async () => {
      ...
      if (path === "") {
        ...
        const done = `- [x] Done \u2705 ${(0, import_obsidian7.moment)().format("YYYY-MM-DD")}
`;
        if (!donePage) {
          donePage = await this.app.vault.create(`${this.configFolder}Process Done.md`, done);
        } else if (donePage instanceof import_obsidian7.TFile && !(await this.app.vault.cachedRead(donePage)).includes(done, 0))
...
var ContextGraph = class extends import_graphology.default {
  constructor(app, officePages, options) {
    ...
    this.setOfficePages = (officePages) => {
      const filteredPages = officePages.filter((p) => this.app.vault.getAbstractFileByPath(p) instanceof import_obsidian8.TFolder);
  ...
  constructor(app, graph) {
    ...
    this.failTask = async (file) => {
      ...
      await this.app.vault.process(file, (fileContent) => {
        ...
        const today = (0, import_obsidian9.moment)().set({ hour: 0, minute: 0, second: 0, millisecond: 0 });
        for (let task of fileTasks) {
          if (task.dueDate && (0, import_obsidian9.moment)(task.dueDate).isBefore(today)) {
            let updatedTask = task.task.replace("[ ]", "[-]") + ` \u274C ${task.dueDate}`;
            let tempTask = task.task;
            if (task.reccuringType != "none") {
              hasReccuringTasks = true;
              const nextDueDate = (0, import_obsidian9.moment)(task.dueDate);
              const nextScheduledDate = task.scheduledDate ? (0, import_obsidian9.moment)(task.scheduledDate) : void 0;
              const nextStartDate = task.startDate ? (0, import_obsidian9.moment)(task.startDate) : void 0;
...
var SnoozeModal = class extends import_obsidian10.FuzzySuggestModal {
  constructor(app) {
    super(app);
    this.onChooseItem = (item, _) => {
      if (!this.app.workspace.getActiveFile())
        return;
      const startDate = (0, import_obsidian10.moment)();
      const endDate = (0, import_obsidian10.moment)();
      if (item.value == 0) {
...
// main.ts
var PersonalOS = class extends import_obsidian11.Plugin {
  constructor() {
    super(...arguments);
    this.loadCommands = () => {
      this.addCommand({
