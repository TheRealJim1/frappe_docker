...
var UNITADE_SETTINGS_TAB = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName(SETTING_LOCALES.getDbSp().name).setDesc(SETTING_LOCALES.getDbSp().desc).setTooltip("This settings registries empty extension, which could be done manually within extension settings block.").addToggle((toggle) => {
      toggle.setValue(this.plugin.settings.barefiling).onChange((value) => __async(this, null, function* () {
        let next = __spreadProps(__spreadValues({}, this.plugin.settings), {
          barefiling: value
        });
        yield this.plugin.uptSettings(next);
    ...
    let groupMsg = new import_obsidian.Setting(containerEl).setName(SETTING_LOCALES.getGrpInf().name).setDesc(SETTING_LOCALES.getGrpInf().desc).setTooltip("For list of views view the docs of the plugin, more information on the wiki.").addToggle((toggle) => {
...
// source/components/files-edit.ts
var import_obsidian5 = require("obsidian");
var TFilesEdit = class extends import_obsidian5.Modal {
  constructor(plugin, target) {
    var _a;
    super(plugin.app);
    this.plugin = plugin;
    this.target = target;
    this._new_extension = "md";
    (_a = this.target) != null ? _a : this.target = [this.plugin.app.vault.getRoot()];
    this._integration = false;
  }
  onOpen() {
    ...
    new import_obsidian5.Setting(contentEl).setName(MODALES_LOCALE.gtToggle1().name).setDesc(MODALES_LOCALE.gtToggle1().desc).addToggle((toggle) => {
      toggle.setValue(this._integration).onChange((value) => __async(this, null, function* () {
        this._integration = value;
      }));
      return toggle;
  ...
  __submit() {
    return __async(this, null, function* () {
      ...
      this.target.forEach((file) => __async(this, null, function* () {
        const filename = file.path.split("/").last();
        const filepath = file.path.split("/").slice(0, -1).join("/");
        const name = filename.split(".").first();
        yield this.app.vault.rename(file, this.__pathgen(filepath, name));
      }));
    });
  }
  __pathgen(path, name) {
...
var TFolderEdit = class extends import_obsidian6.Modal {
  constructor(plugin, target) {
    var _a;
    super(plugin.app);
    this.plugin = plugin;
  ...
  onOpen() {
    ...
    const form = contentEl.createEl("div");
    const disp = contentEl.createEl("span");
    const input = new import_obsidian6.TextComponent(form);
    contentEl.style.cssText = `
            display:          flex;
            align-items:    center;
            flex-direction: column;
    ...
    new import_obsidian6.ButtonComponent(form).setCta().setIcon("pencil").setButtonText("Edit").onClick(() => this.__submit());
  }
  onClose() {
    const { contentEl } = this;
    contentEl.empty();
...
// source/components/files-rename.ts
var import_obsidian8 = require("obsidian");
var TFilesRename = class extends import_obsidian8.Modal {
  constructor(plugin, target) {
    var _a;
    super(plugin.app);
    this.plugin = plugin;
  ...
  onOpen() {
    ...
    const form = contentEl.createEl("div");
    const disp = contentEl.createEl("span");
    const input = new import_obsidian8.TextComponent(form);
    contentEl.style.cssText = `
            display:          flex;
            align-items:    center;
            flex-direction: column;
    ...
    new import_obsidian8.ButtonComponent(form).setCta().setIcon("pencil").setButtonText("Rename").onClick(() => this.__submit());
  }
  onClose() {
    const { contentEl } = this;
    contentEl.empty();
  ...
  __submit() {
    return __async(this, null, function* () {
      ...
      this.target.forEach((file) => __async(this, null, function* () {
        ...
        yield this.app.vault.rename(file, destination);
      }));
    });
  }
  __pathgen(path, extension) {
...
// source/main.ts
var UNITADE_PLUGIN = class _UNITADE_PLUGIN extends import_obsidian9.Plugin {
  constructor() {
    super(...arguments);
  ...
  get is_mobile() {
    return import_obsidian9.Platform.isMobile && this.settings.mobile_settings.enable;
  }
  onload() {
    return __async(this, null, function* () {
  ...
  __ctxEditExts() {
    return this.app.workspace.on("files-menu", (menu, files) => {
      menu.addItem((item) => {
        ...
        item.setIcon("pencil").onClick(() => {
          new TFilesEdit(this, files).open();
        });
      }).addItem((item) => {
        item.setTitle("Rename multiple files");
        item.setIcon("pencil").onClick(() => {
          new TFilesRename(this, files).open();
        });
      });
    });
  }
